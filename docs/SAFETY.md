...
Recycle Bin API. A failure is reported as a failure; it never falls back to permanent deletion.
...
Before either operation, DUPESPACE revalidates the target and its keeper. Checks cover the
physical path, file type, identity, byte size, modification time, and SHA-256. Any changed item is
skipped.
...
Every outcome is written as UTF-8 CSV with timestamp, source, operation mode, status, name/path,
size, checksum, and reason. Trash recovery depends on Windows Recycle Bin behavior. Permanent
deletion has no recovery path.
...
file; rescan before retrying so current state is evaluated again.

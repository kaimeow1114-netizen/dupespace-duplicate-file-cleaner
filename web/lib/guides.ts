  ...
  "zh-TW": [
    {
      ...
      description: "照片看起來一樣，檔案內容卻可能不同。了解精確重複搜尋、壓縮與中繼資料差異，先在瀏覽器比對且不把檔案傳送至伺服器，再安全整理相簿。",
      sections: [
        ...
        { title: "先用不修改檔案的方式檢查", paragraphs: ["開啟本機分析，選擇一個照片資料夾。資料只在自己的瀏覽器中處理，不需登入或上傳；分析會先比對大小，再檢查候選檔案的完整內容指紋。大照片與大型相簿需要實際讀取資料，因此速度也受裝置與儲存媒體影響。", "結果每組只顯示一張代表性圖片，搭配副本名稱與相對路徑。部分格式只會出現類型圖示，這不影響內容比對。比對基準會依瀏覽器可取得的修改時間與路徑排序，不代表已證實那一份是最初拍攝的原檔。"] },

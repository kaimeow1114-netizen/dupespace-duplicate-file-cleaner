export function contentSecurityPolicy(nonce: string, privateWorkspace: boolean): string {
  ...
  return `default-src 'self'; script-src ${scripts}; worker-src 'self'; style-src 'self' 'unsafe-inline'; ${resources} object-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; upgrade-insecure-requests`;
}

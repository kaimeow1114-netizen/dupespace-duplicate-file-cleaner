...
export default function CleanerPage() {
  ...
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb) }} /><SiteHeader /><section className="cleaner-hero"><div className="shell"><span className="eyebrow light"><i /> 瀏覽器直接使用</span><h1>掃描、檢查，一次移至垃圾桶。</h1><p>已登入的使用者只需按「開始掃描」，再按「移至 Google Drive 垃圾桶」。DUPESPACE 會自動選取符合安全門檻的重複副本，並提供完整路徑、檔案預覽與資料夾雙樹鏡像比對；每組至少保留一份，專案與非本人擁有項目硬性排除。</p></div></section><section className="shell cleaner-wrap"><CleanerClient /></section><SiteFooter /></main>;
}

...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="section shell" id="features"><div className="section-heading"><span className="eyebrow"><ShieldCheck size={15} aria-hidden="true" /> SAFE BY DESIGN</span><h2><span className="heading-phrase">讓每次清理，</span><wbr /><span className="heading-phrase">都有清楚的安全邊界。</span></h2><p>內容相同只能成為候選。DUPESPACE 先保護用途、來源與目錄情境，再讓使用者決定要處理哪一份。</p></div><SafetyMotionGrid /></section>
      ...
      <section className="section shell retention-section" id="insights"><div className="section-heading"><span className="eyebrow"><TrendingUp size={15} aria-hidden="true" /> STORAGE INTELLIGENCE</span><h2><span className="heading-phrase">把一次整理，</span><wbr /><span className="heading-phrase">變成持續可見的成果。</span></h2><p>僅保存不含檔名、路徑或檔案 ID 的裝置端彙總數據；趨勢與費用均為整理參考，不代表硬碟健康診斷或實際帳單節省。</p></div><div className="retention-grid"><article><TrendingUp aria-hidden="true" /><b>儲存健康趨勢</b><p>以重複容量與群組數建立 0 至 100 的整理指標，清理後立即更新。</p></article><article><BarChart3 aria-hidden="true" /><b>重複成因分析</b><p>依路徑線索推估下載、通訊軟體暫存、跨資料夾複製與其他來源。</p></article><article><Settings2 aria-hidden="true" /><b>自訂防護設定檔</b><p>保存軟體專案與影音備份等常用保護規則，專案排除仍是硬性安全底線。</p></article></div></section>

...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="section privacy-band"><div className="shell privacy-feature"><div><span className="eyebrow"><ShieldCheck size={15} aria-hidden="true" /> GOOGLE DRIVE PRIVACY</span><h2><span className="heading-phrase">檔案留在你的</span><wbr /><span className="heading-phrase" lang="en">Google Drive。</span></h2><p>DUPESPACE 不下載或代理你的檔案內容。線上掃描只透過 Google Drive API 取得必要的檔案 ID、路徑、版本、大小、權限與校驗碼。</p><div className="privacy-actions"><a className="button primary" href="/cleaner">開啟線上清理器</a><a className="text-link" href="/privacy">閱讀隱私權政策</a></div></div><div className="privacy-card"><b>Google Limited Use 承諾</b><p>DUPESPACE 對 Google API 使用者資料的使用遵守 Google API Services User Data Policy 及 Limited Use 要求。資料不出售、不提供給 AdSense、不用於廣告個人化，也不會用於使用者要求以外的目的。</p><p>加密的 Secure、HttpOnly Cookie 最多維持登入 30 天；登出時會撤銷 Google 權杖並清除工作階段。</p></div></div></section>
      ...
      <section className="section faq-band" id="faq"><div className="shell faq-section"><div className="section-heading"><span className="eyebrow"><ShieldCheck size={15} aria-hidden="true" /> 常見問題</span><h2><span className="heading-phrase">開始之前，</span><wbr /><span className="heading-phrase">先把安全規則說清楚。</span></h2></div><details><summary>DUPESPACE 的用途是什麼？</summary><p>用來搜尋、檢查並安全清理 Windows 與 Google Drive 中內容完全相同的重複檔案與重複資料夾，協助回收儲存空間。</p></details><details><summary>重複資料夾會怎麼處理？</summary><p>只有相對路徑、檔案大小與每個內容校驗碼都一致的完整鏡像才會列出。資料夾只能移至垃圾桶，不能永久刪除；操作前內容若有變更會立即取消。</p></details><details><summary>不同專案中的相同設定檔會被處理嗎？</summary><p>不會。Git、SVN、常見建置專案、套件目錄與虛擬環境會被硬性排除，不列為候選。</p></details><details><summary>Google Drive 檔案內容會上傳嗎？</summary><p>不會。比對只使用 Google Drive API 提供的必要中繼資料與校驗碼。</p></details></div></section>

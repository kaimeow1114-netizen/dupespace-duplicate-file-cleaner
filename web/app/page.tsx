    ...
    <main>
      ...
      <section className="hero shell">
        <div className="hero-copy">
          ...
          <p>免費、開源的 Windows 與 Google Drive 重複檔案清理工具。先看完整路徑與內容比對結果，再把你選定的副本移至垃圾桶。</p>
          <div className="hero-actions">
            <a className="button primary" href={download}>下載 Windows 版 <span>→</span></a>
            <a className="button secondary" href={repo}>在 GitHub 查看原始碼</a>
            <a className="button secondary" href="/cleaner">線上清理 Google Drive</a>
          </div>
      ...
      <section className="section shell" id="features">
        <div className="section-heading"><span className="eyebrow"><i /> Designed to make accidental deletion difficult</span><h2>設計目標，就是讓誤刪變得困難。</h2><p>內容相同只代表「重複候選」。DUPESPACE 會把用途、位置與檔案變更納入保護流程。</p></div>
        <div className="safety-grid">
          ...
          <article><b>03</b><h3>移除前完整檢查</h3><p>先看路徑、大小、群組與容量；專案、程式、備份及同步資料夾預設鎖定。</p></article>
          <article><b>04</b><h3>檔案變更就中止</h3><p>操作前重新驗證路徑、大小、時間與完整校驗碼，掃描後有變化就安全跳過。</p></article>
        </div>
      </section>

      <section className="section alternate"><div className="shell split"><div><span className="eyebrow light"><i /> 內容級精確比對</span><h2>比對檔案內容，不靠檔名猜測。</h2><p>Windows 先按大小分組，再使用完整 SHA-256 內容雜湊。0 位元組忽略，小於 1 MiB 可查看但不會預先勾選。</p><a className="button mint" href="/support">了解安全整理方式</a></div><ol className="steps"><li><span>1</span><div><b>指定保留區</b><small>重要來源與原始資料永遠保護</small></div></li><li><span>2</span><div><b>指定清理區</b><small>只找已有保留區副本的檔案</small></div></li><li><span>3</span><div><b>檢查後移至垃圾桶</b><small>高風險永久刪除另行確認</small></div></li></ol></div></section>

      <section className="section shell privacy-feature"><div><span className="eyebrow"><i /> GOOGLE DRIVE PRIVACY</span><h2>你的檔案留在 Google Drive。</h2><p>DUPESPACE 不會把檔案內容下載到我們的伺服器。重複比對使用 Google Drive API 提供的校驗碼與必要中繼資料；登入權杖存放於加密的 HttpOnly Cookie。</p><a className="button primary" href="/cleaner">開啟線上清理器</a></div><div className="privacy-card"><b>技術細節</b><p>掃描證明 30 分鐘後失效；每次垃圾桶或永久刪除前，重新檢查檔案 ID、版本、所有權、權限、修改時間與校驗碼。</p></div></section>

      <section className="section trust-section"><div className="shell"><div className="section-heading"><span className="eyebrow"><i /> 為什麼選擇 DupeSpace？</span><h2>安全主張，可以直接看程式碼驗證。</h2></div><div className="trust-table" role="table" aria-label="傳統清理工具與 DupeSpace 比較"><div role="row"><b role="columnheader">傳統清理工具</b><b role="columnheader">DUPESPACE</b></div><div role="row"><span>可能只看檔名</span><span>比對檔案內容</span></div><div role="row"><span>可能直接刪除</span><span>預設移至垃圾桶</span></div><div role="row"><span>黑盒操作</span><span>每一步可檢查與匯出 CSV</span></div><div role="row"><span>雲端也要安裝</span><span>Google Drive 可直接使用網頁版</span></div><div role="row"><span>無法驗證宣稱</span><span>完全開源</span></div></div><p className="open-source-line" lang="en">Read the code. Verify the claims. Run it yourself.</p></div></section>

      <section className="section shell faq-section" id="faq"><div className="section-heading"><span className="eyebrow"><i /> 常見問題</span><h2>刪除之前，先把安全規則說清楚。</h2></div><details><summary>DUPESPACE 會直接永久刪除檔案嗎？</summary><p>不會。預設永遠是移至垃圾桶。永久刪除是獨立高風險選項，必須重新手動選取，且警告無法停用。</p></details><details><summary>為什麼 Windows 版要選保留區與清理區？</summary><p>因為內容相同不等於另一份沒有用途。只有清理區檔案在保留區已有相同內容時才會列出，保留區檔案永遠不能刪除。</p></details><details><summary>Google Drive 檔案內容會上傳嗎？</summary><p>不會。比對使用 Google 提供的校驗碼；檔案內容不會上傳到 DUPESPACE 伺服器。</p></details></section>

      <section className="section shell download-section" id="download"><div className="download-card"><Image src="/dupespace-icon.png" alt="" width={140} height={140} /><div><span className="eyebrow"><i /> Windows 10 / 11</span><h2>免費下載 DupeSpace。</h2><p>安裝時可選擇建立桌面捷徑；支援大量結果分頁、批次安全停止與 CSV 稽核報告。</p></div><a className="button primary" href={download}>下載安裝程式</a></div></section>
      <AdPanel />
      <SiteFooter />
    </main>
  );

...
export const metadata = chineseMetadata("support", "重複檔案與 Windows 儲存空間整理指南", "了解如何安全分析與清理本機重複檔案、照片影片與備份資料夾，以及何時應改用 Windows 儲存空間感知。");

export default function SupportPage() {
  return (
    <main>
      <SiteHeader pagePath="support" />
      ...
      <section className="shell guide-grid">
        ...
        <article><span>02</span><h2>需要時再保護子資料夾</h2><p>Windows 版會先套用保護規則，再依可判定的建立時間與路徑選出保留建議。若原始照片或工作文件集中在特定子資料夾，也能把它設為永遠不可勾選的保護資料夾。</p></article>
        <article><span>03</span><h2>看內容，不只看檔名</h2><p>相同檔名可能是不同版本；不同檔名也可能內容完全一致。網頁版在裝置端做分塊內容比對，Windows 版為大量與大型檔案最佳化，但仍應查看完整路徑與用途。</p></article>
        <article><span>04</span><h2>程式碼專案一律硬性保護</h2><p>Git／SVN、常見建置專案、套件目錄與虛擬環境中的設定、依賴或外掛可能被不同專案各自需要，Windows 版偵測到這些情境時不列為清理候選。自訂專案仍須自行核對與保護。執行檔、備份與同步資料夾則維持預設鎖定。</p></article>

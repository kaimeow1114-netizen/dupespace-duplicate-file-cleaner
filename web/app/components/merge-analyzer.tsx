...
function FolderPicker({ side, value, onChange, disabled, locale }: {
  ...
  return <section className={`merge-folder-card ${incoming ? "incoming" : "destination"}`}>
    <input ref={input} className="sr-only" type="file" multiple disabled={disabled}
      aria-label={en ? `Choose ${incoming ? "incoming" : "destination"} folder` : `選擇${incoming ? "待合併" : "目的"}資料夾`}
      onChange={(event) => { onChange(selectedFolder(Array.from(event.target.files ?? []), side)); event.target.value = ""; }}
      {...({ webkitdirectory: "", directory: "" } as Record<string, string>)} />
    <div className="merge-folder-heading"><span>{incoming ? "01" : "02"}</span><div><small>{en ? (incoming ? "INCOMING" : "DESTINATION") : (incoming ? "準備匯入" : "既有收藏")}</small><h2>{en ? (incoming ? "Folder to merge" : "Destination folder") : (incoming ? "待合併資料夾" : "目的資料夾")}</h2></div></div>
...
function FilePaths({ records, side, locale }: { records: MergeRecord[]; side: MergeSide; locale: "zh-TW" | "en" }) {
  ...
  return <div className={`merge-path-column ${incoming ? "incoming" : "destination"}`}>
    <b>{en ? (incoming ? "Incoming" : "Destination") : (incoming ? "待合併" : "目的資料夾")}</b>
...
function categoryCopy(category: MergeCategory, en: boolean): { title: string; description: string } {
  const values = {
    ...
    same_path_same_content: en ? ["Identical at the same location", "The destination already contains this exact content at the same relative path."] : ["同位置、同內容", "目的資料夾的相同位置已有完全一致的內容。"],
    incoming_only: en ? ["New incoming file", "No exact content match was found in the destination."] : ["待加入的新檔案", "目的資料夾尚未找到完全相同的內容。"],
    destination_only: en ? ["Destination only", "This file exists only in the destination folder."] : ["只存在於目的資料夾", "待合併資料夾中沒有這份內容。"],
  } as const;
...
export function MergeAnalyzer({ locale = "zh-TW" }: { locale?: "zh-TW" | "en" }) {
  ...
  async function analyze(): Promise<void> {
    ...
    try {
      ...
      if (controller.current !== run) return;
      setResult(compared); setProgress(100); setState("done"); setStatus(en ? "Comparison complete. No files were changed." : "核對完成，沒有變更任何檔案。");
    } catch {
      if (controller.current !== run) return;
  ...
  const decision = result?.summary.conflictPaths
    ...
    : result?.summary.reviewFindings
      ...
      : result
        ? { tone: "ready", icon: CheckCircle2, title: en ? "The merge map is ready for review." : "合併地圖已完成，可以開始依分類檢查。", body: en ? "No same-path conflicts or known context risks were detected. The result remains advisory and no files were changed." : "未偵測到同路徑版本衝突或已知用途風險；結果仍是決策參考，沒有變更任何檔案。" }
        : null;
  ...
  return <section className="merge-workbench">
    ...
    <div className="merge-action-bar"><span><ShieldCheck aria-hidden="true" />{en ? "Read by this tab on-device. Files are never sent to a server or changed." : "只授權此分頁在裝置端讀取，不會傳送至伺服器或更動檔案。"}</span><div>{result && <button className="button secondary" type="button" onClick={exportReport}><Download aria-hidden="true" />{en ? "Export CSV" : "匯出 CSV"}</button>}<button className="button primary" type="button" disabled={!incoming || !destination || state === "scanning"} onClick={() => void analyze()}>{result ? <RefreshCcw aria-hidden="true" /> : <FileSearch aria-hidden="true" />}{result ? (en ? "Compare again" : "重新核對") : (en ? "Compare folders" : "核對兩個資料夾")}</button></div></div>
    ...
    {(state === "stopped" || state === "error") && <div className="merge-message error"><AlertCircle aria-hidden="true" /><div><b>{state === "error" ? (en ? "Comparison could not finish" : "核對未能完成") : (en ? "Comparison stopped" : "核對已停止")}</b><p>{status}</p></div></div>}
    {result && <div className="merge-results">
      <div className="merge-summary"><article><FolderIncoming aria-hidden="true" /><small>{en ? "Avoid copying again" : "可避免再次複製"}</small><strong>{result.summary.avoidCopyFiles.toLocaleString()}</strong><span>{formatBytes(result.summary.avoidCopyBytes)}</span></article><article><FolderOutput aria-hidden="true" /><small>{en ? "New incoming files" : "待加入的新檔案"}</small><strong>{result.summary.incomingOnlyFiles.toLocaleString()}</strong><span>{en ? "No exact match" : "目的端無相同內容"}</span></article><article className={result.summary.conflictPaths ? "urgent" : ""}><FileQuestion aria-hidden="true" /><small>{en ? "Version conflicts" : "版本衝突"}</small><strong>{result.summary.conflictPaths.toLocaleString()}</strong><span>{en ? "Review before merging" : "合併前必須確認"}</span></article><article className={result.summary.reviewFindings ? "warning" : ""}><ShieldAlert aria-hidden="true" /><small>{en ? "Context review" : "用途風險確認"}</small><strong>{result.summary.reviewFindings.toLocaleString()}</strong><span>{en ? "Projects, apps or backups" : "專案、程式或備份"}</span></article></div>
      ...
      <div className="merge-result-heading"><div><span className="eyebrow">{en ? "MERGE MAP" : "合併地圖"}</span><h2>{en ? "See what will happen before you merge." : "合併之前，先知道會發生什麼。"}</h2><p>{en ? `${result.summary.incomingFiles.toLocaleString()} incoming and ${result.summary.destinationFiles.toLocaleString()} destination files analyzed. ${result.summary.ignoredEmptyFiles.toLocaleString()} empty files ignored.` : `已核對 ${result.summary.incomingFiles.toLocaleString()} 個待合併檔案與 ${result.summary.destinationFiles.toLocaleString()} 個目的端檔案；忽略 ${result.summary.ignoredEmptyFiles.toLocaleString()} 個空白檔案。`}</p></div><span><CheckCircle2 aria-hidden="true" />{en ? "Read-only result" : "唯讀結果"}</span></div>
      <div className="merge-filters" role="tablist" aria-label={en ? "Filter comparison results" : "篩選核對結果"}>{filters.map((value) => { const label = value === "all" ? (en ? "All" : "全部") : categoryCopy(value, en).title; const count = value === "all" ? result.findings.length : counts.get(value) ?? 0; return <button key={value} type="button" role="tab" aria-selected={filter === value} disabled={!count} onClick={() => { setFilter(value); setPage(0); }}>{label}<span>{count}</span></button>; })}</div>
      {shown.length ? <div className="merge-findings">{shown.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE).map((item) => <FindingCard key={item.id} finding={item} locale={locale} />)}</div> : <div className="merge-message"><CheckCircle2 aria-hidden="true" /><div><b>{en ? "No results in this category" : "這個分類沒有項目"}</b><p>{en ? "Choose another filter to continue reviewing the merge map." : "可切換其他分類繼續查看合併地圖。"}</p></div></div>}
      {shown.length > PAGE_SIZE && <nav className="merge-pagination" aria-label={en ? "Comparison result pages" : "核對結果分頁"}><button className="button secondary" type="button" disabled={page === 0} onClick={() => setPage((value) => value - 1)}>{en ? "Previous" : "上一頁"}</button><span>{page + 1} / {Math.ceil(shown.length / PAGE_SIZE)}</span><button className="button secondary" type="button" disabled={(page + 1) * PAGE_SIZE >= shown.length} onClick={() => setPage((value) => value + 1)}>{en ? "Next" : "下一頁"}</button></nav>}
    </div>}
  </section>;
}
